...
The one artifact here that is **partly** not ingested from a vendor at all.
Its summary half is produced from the reports this repo already renders, by
`espn_ff summarize`, and has no upstream cadence to report: it exists when a
report exists and a generation succeeded. Its `news` half is the opposite —
the only thing in this document sourced from the open web, via Google Search
grounding, with no feed, no schedule and no artifact timestamp behind it.
`docs/ai-summaries.md` owns the prompts, the triggering and the two cost
meters; this section covers only the stored fields.

### `summaries/<season>/week-NN/<YYYY-MM-DD>-<day>-<slug>.json` — `ai/summarize.envelope` (16 fields)
...
| Field | Meaning | Upstream release cadence | Our ingest |
|---|---|---|---|
| `schema_version` | `2`. Bumped if the shape below changes incompatibly. v1 → v2 added `news` and `news_error` and moved nothing | — | — |
...
| `news` | The grounded roster-news block, or **`null`** when the grounded calls failed or were skipped. Null is not an error on its own — see `news_error` | Whatever the open web published | Regenerated by the next run while it is null |
| `news_error` | Why `news` is null, or null itself. Populated means it **failed** and will be retried; null beside a null `news` means it was **skipped** (`--no-news`). The distinction is the whole reason this is a separate field | — | — |
| `news.generated_at` / `.model` / `.prompt_sha256` | The news layer's own provenance, separate from the summary's. On a news-only backfill the top-level `generated_at` and `prompt_sha256` keep describing the summary, and only these move | — | — |
| `news.roster_week` / `.roster_export` | Which roster snapshot the news describes: the report's week, and the `weekly-rosters` export filename it was sliced from. **This is the closest thing to a freshness field the news half has** — the news itself carries no artifact timestamp | — | — |
| `news.search_query_count` | Grounded searches the model chose to run, summed across groups. A **billing** figure, not a quality one: 40 searches is more expensive than 12, not better-researched. Metered against 5,000 free/month — see `docs/ai-summaries.md` | — | — |
| `news.players[]` | One object per rostered player, **always** — reconciled against the roster in code, so the array length equals the roster size. Identity fields (`player_name`, `position`, `pro_team`, `lineup_slot`, `espn_injury_status`) come from the roster row, never from the model | — | — |
| `news.players[].found` | `false` means *searched and nothing turned up*, with `headline` / `detail` / `as_of` all null and a `note` saying which. It does **not** mean the player was missed — a missed player is impossible by construction | — | — |
| `news.players[].as_of` | The item's own date, or **null by design**. The model is instructed to leave it empty rather than default to today, so a null here is a stated unknown and not a gap | — | — |
| `news.groups.<group>` | Per-call detail for `starters` / `bench` / `ir`: `grounded`, `search_queries`, `sources` (`uri` + `title`, from the API's `groundingMetadata`, never model-authored), `search_entry_point` (Search Suggestions HTML, stored because Google's terms require displaying it), and `usage`. A group with nobody in it carries `skipped` instead and cost no call | — | — |
| `news.grounded` / `news.groups.<group>.grounded` | **Whether a search actually ran.** Measured from the response, never asserted. ~1 call in 4 returns no grounding metadata and answers from recall instead (Observed 2026-09-16); the call is retried once, and if it still comes back unsourced the items are kept but flagged. `false` means *treat these claims as the model's own*, and the group's `sources` will be empty. The top-level value is false if **any** group that ran was ungrounded; a skipped group has no opinion | — | — |
...
**That argument covers the summary half and does not cover the news half,**
which is worth stating rather than letting the paragraph above imply
otherwise. News genuinely can go stale in the ordinary sense: a practice
report fetched for Wednesday's envelope is three days old by the Sunday
pre-lock call, and no hash detects that, because nothing about it changed —
the world did. The mitigations are weaker and are all the reader's to apply:
`news.generated_at` says when it was fetched, `news.roster_week` and
`.roster_export` say which snapshot it describes, and each
`players[].as_of` dates the item itself where the source allowed it. A
reader comparing `news.generated_at` against today is doing the work the
`report.sha256` trick does automatically for the other half. There is no
per-envelope refresh: news is generated once and never revisited, so the
Sunday report's envelope carries Sunday's news and Wednesday's carries
Wednesday's.
