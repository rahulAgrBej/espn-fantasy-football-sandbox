...
def _events_by_team(events_payload):
    """ESPN team abbreviation -> event_id, from a free /events response."""
    index = {}
    for event in events_payload or []:
        index[normalize_team(event.get("home_team"))] = event.get("id")
        index[normalize_team(event.get("away_team"))] = event.get("id")
    return index
...
def decision_events(espn_client, events_payload, team_id, week=None, commence_after=None, event_ids=None):
    ...
    event_by_team = _events_by_team(events_payload)
